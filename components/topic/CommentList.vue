<template>
  <div>
    <div v-for="(comment, index) in commentList" :key="index" class="comment">
      ...
      />
      <div class="reply-list" @click="$router.push(`/thread/comment?threadId=${threadId}&commentId=${comment._jv.id}#reply`)">
...
export default {
  ...
  methods: {
    ...
    showAll(e, comment) {
      if (e.target.matches('.showAllComment')) this.$router.push(`/thread/comment?threadId=${this.threadId}&commentId=${comment._jv.id}`)
    },
    replyPublish(id) {
